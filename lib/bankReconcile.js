...
// Deux chemins mènent au rapprochement automatique.
//
// 1. LA CERTITUDE — montant exact ET commerçant identique. Elle se suffit à
//    elle-même, sans condition de date : un ticket de caisse peut être déposé
//    des semaines après l'achat, et la date du relevé n'apprend rien de plus
//    quand l'enseigne et le centime concordent déjà.
//
// 2. LE SCORE, pour tout le reste. Un score de 8 exige, selon le type :
//     - facture (fournisseur/émise) : montant exact + référence QR ou IBAN,
//     - frais : montant exact + commerçant + date proche.
//    Une seule preuve « faible » ne suffit jamais.
...
export function planAutoReconcile(transactions, candidates) {
  ...
  for (const tx of transactions) {
    ...
    // Chemin 1 : la certitude. Elle court-circuite le score ET l'écart — mais
    // seulement si elle désigne UN candidat. Deux documents au même montant
    // chez le même commerçant, c'est exactement le piège du double paiement :
    // celui-là se tranche à la main.
    const certains = suggestions.filter(s => s.certain)
    if (certains.length === 1) {
      const c = certains[0]
      taken.add(`${c.type}#${c.candidate.id}`)
      matched.push({ tx, type: c.type, candidate: c.candidate, score: c.score, reasons: c.reasons })
      continue
    }
    if (certains.length > 1) {
      ambiguous.push({ tx, candidates: certains.slice(0, 2) })
      continue
    }

    // Chemin 2 : le score, avec l'écart qui départage.
    if (best.score < AUTO_MIN_SCORE) continue
