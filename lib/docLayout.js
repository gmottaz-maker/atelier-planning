...
export function lineTable(rows, { summary = false } = {}) {
  ...
  const body = rows.map(r => {
    ...
    if (summary) return ''
    // Le gras ne marque que l'item ; ses composants se lisent comme une liste.
    // Le retrait et la graisse suffisent à les distinguer, sans que la page
    // devienne un mur de gras.
    const sous = r.level === 2
    const pad = sous ? 'padding-left:14px;' : ''
    return `<tr style="border-bottom:1px solid ${HAIR}">
      <td style="padding:14px 0">
        <div style="${pad}font-weight:${sous ? 400 : 700}">${esc(r.title)}</div>
