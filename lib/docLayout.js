...
export const DOC_CSS = `
  * { box-sizing: border-box; }
  ...
  /* À L'ÉCRAN : on dessine une feuille, marges comprises, pour l'aperçu. */
  .doc .page { width:210mm; min-height:297mm; margin:0 auto; }
  .doc .content { padding:18mm 18mm 14mm; }

  /* À L'IMPRESSION : les marges viennent de @page, et de là seulement.
     Elles étaient auparavant un padding sur .content — or un padding ne
     s'applique qu'au DÉBUT et à la FIN du bloc, jamais entre deux pages :
     à partir de la page 2, le texte touchait le bord du papier. */
  @media print {
    .doc .page { width:auto; min-height:0; margin:0; }
    .doc .content { padding:0; }

  }

  /* Coupures de page.
     Une ligne ne se coupe jamais ; un titre de section ne reste jamais seul en
     bas de page ; et le bas du document — conditions, totaux, pied — tient
     d'un bloc : des totaux séparés de leur montant sont illisibles. */
  .doc tr { break-inside:avoid; }
  .doc tr.sec { break-after:avoid; }
  .doc thead { display:table-header-group; }
  .doc .bas-document { break-inside:avoid; }
  .doc .doc-footer { break-inside:avoid; break-before:avoid; }
`
...
export function lineTable(rows) {
  ...
  // Retrait par niveau, graisse par rôle : une composition n'est jamais en
  // gras, qu'elle tienne sous un item (niveau 2) ou sous un élément (niveau 3).
  const RETRAIT = { 1: 0, 2: 14, 3: 28 }
  const GRAISSE = { item: 700, element: 600, prestation: 700, composition: 400 }
  ...
  const body = rows.map(r => {
    if (r.kind === 'section') {
      ...
      const pad = `padding-left:${RETRAIT[r.level] ?? 0}px;`
    const poids = GRAISSE[r.role] ?? 400

    // Escompte : la position au-dessus porte son montant PLEIN, cette ligne
    // dit ce que le rabais retire, et la suivante le net. Sans elles, Prix ×
    // Qté ne retombait pas sur le sous-total et le client lisait une erreur.
    // Pas de filet sous ces deux lignes : la position et son escompte forment
    // un bloc, que le trait de séparation couperait en deux.
    if (r.role === 'escompte') {
      return `<tr>
        <td style="padding:2px 0 0"><div style="${pad}font-weight:400;color:${GREY};font-size:11.5px">${esc(r.title)}</div></td>
        <td></td><td></td>
        <td style="text-align:right;${cell}color:${GREY};padding:2px 0 0">− ${fmtCHF(Math.abs(r.total))}</td>
      </tr>`
    }
    if (r.role === 'net') {
      return `<tr style="border-bottom:1px solid ${HAIR}">
        <td colspan="3" style="padding:0 0 14px"></td>
        <td style="text-align:right;padding:3px 0 14px">
          <div style="height:1px;background:${INK};margin-bottom:3px"></div>
          <span style="${cell}font-weight:500">${fmtCHF(r.total)}</span>
        </td>
      </tr>`
    }

    return `<tr style="${r.escompte ? '' : `border-bottom:1px solid ${HAIR}`}">
      <td style="padding:14px 0 ${r.escompte ? '0' : '14px'}">
        <div style="${pad}font-weight:${poids}">${esc(r.title)}</div>
...
export function bottomBlock(conditionsHtml, totals) {
  ...
  return `<div class="bas-document" style="display:flex;justify-content:space-between;gap:32px;margin-top:44px">
...
export function docFooter(company) {
  ...
  return `<div class="doc-footer" style="margin-top:28px;padding-top:18px;border-top:1px solid ${HAIR};display:flex;align-items:center;gap:10px">
    ${amazingLogo(20)}
...
/**
 * Page autonome du bulletin QR, produite comme un DOCUMENT SÉPARÉ puis
 * fusionnée au reste (lib/htmlToPdf.js).
 *
 * Pourquoi séparé : les pages de contenu ont besoin de marges qui se répètent,
 * donc de `@page { margin }`. Le bulletin, lui, doit venir mourir sur le bord
 * du papier — il déborderait de la zone de contenu et Chrome le repousserait
 * sur une page blanche. Les pages CSS nommées (`@page bulletin { margin: 0 }`)
 * auraient résolu ça d'un trait, mais Chrome ne les honore pas : vérifié, il
 * insère une page vide.
 */
export function qrDocument(qrSvg) {
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8">
<style>
  @page { size: A4; margin: 0; }
  body { margin:0; }
  .qr { width:210mm; margin-top:192mm; }   /* 297 − 105 : collé au bord bas */
  .qr svg { display:block; width:210mm; height:105mm; }
</style></head><body><div class="qr">${qrSvg}</div></body></html>`
}
...
<style>@page { size: A4; margin: 18mm 18mm 16mm; } body { margin:0; }${DOC_CSS}</style>
