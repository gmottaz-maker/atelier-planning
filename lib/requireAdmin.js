...
// Repli utilisé tant que `profiles.role` n'existe pas (schema-profiles-role.sql
// pas encore joué). Le rôle fait autorité dès que la colonne est là ; ce nom ne
// sert plus qu'à ne pas verrouiller l'admin hors de l'application entre le
// déploiement du code et l'exécution de la migration.
const roleDeNom = name => (name === ADMIN_USER ? 'admin' : 'member')
...
export async function getVerifiedUser(req) {
  ...
  try {
    ...
    // `role` peut ne pas exister encore : on retombe alors sur le nom seul,
    // sans quoi l'erreur de colonne inconnue déconnecterait tout le monde.
    let profile = null
    const avecRole = await supabase
      .from('profiles').select('name, role').eq('id', user.id).maybeSingle()
    if (avecRole.error) {
      const sansRole = await supabase
        .from('profiles').select('name').eq('id', user.id).maybeSingle()
      profile = sansRole.data
    } else {
      profile = avecRole.data
    }
    ...
    const verified = {
      ...
      email: user.email,
      name,
      role: profile?.role || roleDeNom(profile?.name),
    }
...
/** Vrai si l'utilisateur porte le rôle administrateur. */
 ...
 * Le contrôle porte sur le RÔLE, pas sur le nom : renommer un profil ne doit
 * jamais changer ses permissions.
