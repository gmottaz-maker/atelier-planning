...
export function buildDevisBody(project, company) {
  ...
  ${objectBlock(project.name, [
    ...
    // La référence du client — son bon de commande — sous l'objet plutôt que
    // dans l'en-tête : c'est là qu'on la cherche, avec ce à quoi elle se
    // rapporte, et c'est par elle que le client retrouve le dossier.
    project.reference ? `Référence : ${project.reference}` : null,
