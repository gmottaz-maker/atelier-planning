...
import { nature as natureBancaire } from './bankClassification'
...
/**
 * Compte à utiliser pour une catégorie. `defaut` sert de dernier recours quand
 * l'appelant en connaît un meilleur que le repli générique — c'est le cas des
 * natures bancaires, où « autres charges » serait faux pour un salaire.
 */
export function accountFor(mappings, scope, category, defaut) {
  const list = mappings || []
...
export function buildJournal({ customerInvoices = [], supplierInvoices = [], expenses = [], bankTx = [], mappings = [] }) {
  ...
  for (const t of bankTx) {
    ...
    if (t.matched_to_type === 'customer_invoice' && amt > 0) {
      ...
      // Mouvement sans pièce : salaire, virement interne, frais bancaires,
      // impôts. Sans cette branche, ces lignes n'entraient tout simplement pas
      // au journal — le compte de résultat était incomplet.
      const nat = natureBancaire(t.classification)
      if (!nat) continue
      const compte = accountFor(mappings, 'bank', t.classification, nat.compteDefaut)
      const libelle = `${nat.label} — ${lib}`
      // Un débit sort de la banque (compte au débit / banque au crédit) ;
      // un crédit y entre, et l'écriture s'inverse — un remboursement d'impôt
      // ou un virement DEPUIS l'autre compte existent.
      if (amt < 0) push(t.booking_date, piece, libelle, t.counterparty_name, compte, BANK, -amt, { type: nat.label })
      else          push(t.booking_date, piece, libelle, t.counterparty_name, BANK, compte, amt, { type: nat.label })
    }
  }
