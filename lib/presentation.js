...
export async function hydraterImages(deck, telecharger) {
  ...
  // Un même fichier occupe plusieurs emplacements : la paire d'une pièce sert
  // aussi d'illustration à la page « comment lire les visuels », et son premier
  // visuel à l'aperçu. Sur une présentation réelle, 6 fichiers remplissaient 15
  // cadres — téléchargés 15 fois, et recopiés 15 fois dans le document, qui
  // pesait 29,5 Mo pour 7,9 Mo d'images. Chromium abandonnait avant la fin.
  //
  // On ne télécharge donc chaque fichier qu'UNE fois, et tous ses emplacements
  // partagent la même chaîne : le navigateur ne décode l'image qu'une fois.
  const deja = new Map()
  for (const { chemin } of imagesDuDeck(copie)) {
    const cible = emplacementImage(copie, chemin)
    const cle = String(cible.kdrive_id)
    try {
      if (!deja.has(cle)) {
        const { base64, mime } = await telecharger(cible.kdrive_id)
        deja.set(cle, base64 ? `data:${mime || cible.mime || 'image/jpeg'};base64,${base64}` : null)
      }
      const src = deja.get(cle)
      if (src) cible.src = src
    } catch {
