...
// Une activité inconnue est présumée facturée à l'heure : c'est le cas
// général, et l'exception (conduite, interne) est explicite en base.
const factureeAlHeure = (code, activites) => {
  const a = activites.find(x => String(x.code) === String(code))
  return a ? a.facturee_heure !== false : true
}

/**
 * Ce qui a été réellement dépensé et imputé.
 *
 * `heures` ne compte que les activités FACTURÉES À L'HEURE : c'est ce qui se
 * compare aux heures offertes. La conduite, facturée au km, et l'interne vont
 * dans `heuresNonFacturees` — elles coûtent, mais aucune offre ne les prévoit.
 */
export function reelProjet(couts = [], heuresImputees = [], activites = []) {
  ...
  let minutes = 0
  let minutesNonFacturees = 0
  for (const h of heuresImputees) {
    const m = Number(h.minutes) || 0
    if (factureeAlHeure(h.activite, activites)) minutes += m
    else minutesNonFacturees += m
  }
  return {
    ...
    heuresNonFacturees: arrondi(minutesNonFacturees / 60),
  }
}
...
export function mainOeuvreReelle(heures = [], activites = []) {
  ...
  const lignes = Object.entries(minutesPar).map(([code, minutes]) => {
    const a = activites.find(x => String(x.code) === String(code)) || {}
    const h = minutes / 60
    const facturee = a.facturee_heure !== false
    return {
      code: Number(code),
      ...
      // Une activité qui ne se facture pas à l'heure n'a pas de valeur « au
      // tarif » : la conduite se paie au km, pas au temps passé.
      vente: facturee && renseigne(a.tarif_vente) ? arrondi(h * num(a.tarif_vente)) : null,
    }
...
/**
 * Heures offertes et heures passées, ACTIVITÉ PAR ACTIVITÉ.
 *
 * C'est la question de départ de tout le chantier des heures : « on a offert
 * 12 h de peinture, on en a passé 19 ». Le total seul ne la pose pas — un
 * dépassement en peinture compensé par une avance en assemblage donne un écart
 * nul et n'apprend rien.
 *
 * Une ligne d'offre porte son activité dans `activite` (le code, comme les
 * heures imputées et la ligne de compensation du consulting). Une ligne SANS
 * code ne s'invente pas d'activité : ses heures vont dans `prevuSansCode`, et
 * l'écran dit ce qu'il n'a pas pu ventiler. Les offres écrites avant l'arrivée
 * des codes tombent entièrement là — c'est normal, pas une erreur.
 *
 * Seules les lignes EN HEURES comptent, comme dans `prevuDevis` : une journée
 * ne vaut pas le même nombre d'heures pour tout le monde. Côté réel, seules les
 * activités facturées à l'heure — la conduite part au km, l'interne ne se
 * facture pas, aucune offre ne les prévoit.
 */
export function heuresParActivite(raw, heuresImputees = [], activites = []) {
  const q = normaliserDevis(raw)
  const lignesOffre = [...q.management, ...q.logistics]
  for (const it of q.items) {
    lignesOffre.push(...it.labor)
    for (const el of it.elements) lignesOffre.push(...el.labor)
  }

  const prevu = {}
  let prevuSansCode = 0
  for (const r of lignesOffre) {
    if (!enHeures(r.unit)) continue
    const h = num(r.quantity)
    if (!h) continue
    const code = r.activite
    if (code === null || code === undefined || code === '') prevuSansCode += h
    else prevu[code] = (prevu[code] || 0) + h
  }

  const reel = {}
  for (const h of heuresImputees) {
    if (!factureeAlHeure(h.activite, activites)) continue
    reel[h.activite] = (reel[h.activite] || 0) + (Number(h.minutes) || 0) / 60
  }

  const codes = [...new Set([...Object.keys(prevu), ...Object.keys(reel)])]
  const lignes = codes.map(code => {
    const a = activites.find(x => String(x.code) === String(code)) || {}
    const p = arrondi(prevu[code] || 0)
    const r = arrondi(reel[code] || 0)
    return {
      code: Number(code),
      libelle: a.libelle || `activité ${code}`,
      ...
      reel: r,
      ecart: arrondi(r - p),
      // Rien d'offert : un pourcentage d'un zéro ne veut rien dire.
      pct: p ? Math.round((r - p) / p * 100) : null,
    }
  }).sort((x, y) => x.code - y.code)

  return { lignes, prevuSansCode: arrondi(prevuSansCode) }
}

/**
 * Prix de l'offre moins tout ce qui a réellement été dépensé — véhicules
 * compris : un km roulé coûte leasing, assurance et diesel, qu'on le facture
 * ou non (lib/transport.js).
 */
export function margeReelle(venteHT, reel, coutMainOeuvre, coutVehicules = 0) {
  const vente = arrondi(num(venteHT))
  const couts = arrondi((reel?.materiel || 0) + (reel?.sous_traitance || 0) + (reel?.autre || 0)
    + (coutMainOeuvre || 0) + (coutVehicules || 0))
