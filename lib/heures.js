...
// ── Activités dans l'offre ───────────────────────────────────────────────────
//
// Une ligne de main-d'œuvre de l'offre porte le CODE de l'activité qu'elle
// chiffre, dans `activite` — le même champ que les heures imputées et que la
// ligne de compensation du consulting. C'est ce qui permet à la rentabilité de
// dire « peinture : 12 h offertes, 19 h passées » au lieu d'un seul total.
//
// On ne recopie PAS les activités dans le catalogue : deux listes auraient deux
// tarifs, et le jour où l'assemblage passe à 130.–, l'une des deux mentirait.
// L'offre lit directement la liste des activités.
...
export const libelleFamille = f => LIBELLES_FAMILLE[f] || f || 'Autre'

/**
 * Les activités qu'une offre peut chiffrer : actives et facturées à l'heure.
 * La conduite part au km et l'interne ne se facture pas — les proposer ici
 * inviterait à les vendre à l'heure.
 */
export function activitesFacturables(activites = []) {
  return (Array.isArray(activites) ? activites : [])
    .filter(a => a && a.actif !== false && a.facturee_heure !== false)
    .sort((a, b) => a.code - b.code)
}

/** Les mêmes, rangées par famille dans l'ordre des codes. */
export function activitesParFamille(activites = []) {
  const groupes = []
  for (const a of activitesFacturables(activites)) {
    const f = a.famille || 'autre'
    let g = groupes.find(x => x.famille === f)
    if (!g) { g = { famille: f, libelle: libelleFamille(f), activites: [] }; groupes.push(g) }
    g.activites.push(a)
  }
  return groupes
}
...
/**
 * La ligne d'offre qu'une activité remplit.
 *
 * Le libellé est un point de départ : le client lira ce qu'on y écrit, et le
 * code, lui, reste interne. Un tarif absent reste VIDE — un tarif par défaut
 * vendrait la CNC au prix de l'assemblage sans que personne s'en aperçoive.
 */
export function ligneOffreActivite(a) {
  const tarif = a?.tarif_vente
  return {
    description: a?.libelle || '',
    rate: tarif === null || tarif === undefined || tarif === '' ? '' : String(tarif),
    quantity: '',
    unit: 'heure(s)',
    activite: a?.code ?? null,
  }
}

/**
 * Choisir un code sur une ligne doit-il en réécrire le libellé ?
 *
 * Oui quand le libellé est VIDE (la ligne vient d'être créée), ou quand il est
 * encore celui de l'activité PRÉCÉDENTE (on corrige 31 en 23 : « Peinture »
 * doit suivre et devenir « Assemblage »). Non dès qu'on a écrit autre chose —
 * sinon poser un code sur une ancienne ligne effacerait ce qu'on a rédigé pour
 * le client. La plupart du temps, le nom de l'activité suffit ; quand il ne
 * suffit pas, on corrige, et la correction tient.
 */
export function libelleSuitActivite(texte, ancienLibelle) {
  const t = String(texte ?? '').trim()
  if (!t) return true
  return !!ancienLibelle && t === String(ancienLibelle).trim()
}
