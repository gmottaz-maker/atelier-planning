...
/** Clé d'unicité d'une facture de stockage : un client, un trimestre. */
export const storageBillingKey = (year, quarter, client) => `${year}-Q${quarter}-${client}`
...
export async function createStorageInvoices(supabase, year, quarter, { dry } = {}) {
  ...
  for (const p of payloads) {
    ...
    const { data, error: e } = await supabase.from('customer_invoices').insert({
      ...
      object: p.object,
      // Clé métier du trimestre : l'index unique en base est ce qui empêche
      // vraiment le doublon. La lecture de `done` ci-dessus ne protège pas de
      // deux exécutions concurrentes du cron — toutes deux la franchissent.
      storage_billing_key: storageBillingKey(year, quarter, p.client),
    ...
    if (e) {
      // 23505 = violation d'unicité : une exécution concurrente a créé cette
      // facture entre-temps. C'est le résultat voulu, pas un échec.
      if (e.code === '23505') { skipped.push(p.client); continue }
      throw new Error(`${p.client} : ${e.message}`)
    }
    created.push(data)
  }
