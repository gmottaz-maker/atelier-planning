...
// Modes de livraison proposés à la saisie. Ils vivaient dans `pages/index.js`,
// où la fiche projet ne pouvait pas les lire : deux listes auraient divergé au
// premier ajout.
export const TYPES_LIVRAISON = ['Livraison', 'Montage sur place', 'Client vient chercher', 'Enlèvement sur place']

/**
 * Ce qu'on envoie à `PUT /api/projects/[id]` pour modifier l'en-tête d'un
 * projet depuis sa fiche.
 *
 * Deux pièges, tous deux vérifiés par des tests :
 *
 *  1. le projet part ENTIER. La route écrit `deadline: deadline || null` : un
 *     envoi partiel n'oublierait pas l'échéance, il l'EFFACERAIT ;
 *  2. les gros documents — offre, fiche de visite, logistique — sont retirés.
 *     La route ne les écrit que s'ils sont présents, et les renvoyer depuis un
 *     état chargé il y a une heure écraserait le travail fait entre-temps dans
 *     un autre onglet. Modifier une échéance ne doit pas toucher à une offre.
 */
export function chargeUtileEntete(projet, form) {
  const { quote_data, site_visit_data, site_visit_summary, logistics_data, ...reste } = projet || {}
  return {
    ...reste,
    ...(form || {}),
    // Une date vide est une absence d'échéance, pas une chaîne vide.
