...
/**
 * Marge d'un trajet SIMULÉ — l'outil /outils/marge-km, où l'on essaie des
 * hypothèses avant de faire une offre.
 *
 * Contrairement à transportPrevu, un coût inconnu rend `marge: null` plutôt
 * qu'une marge partielle : sur un écran de simulation, un chiffre incomplet
 * qu'on prendrait pour un résultat serait pire que pas de chiffre.
 */
export function simulerTrajet({ prix, km = 0, minutes = null, coutKm = null, personnes = 1, coutHoraire = null, vitesse = null }) {
  const duree = minutes != null ? minutes : (vitesse && km ? km / vitesse * 60 : null)
  const coutVehicule = coutKm == null ? null : arrondi(km * coutKm)
  const coutTemps = duree != null && coutHoraire != null ? arrondi(duree / 60 * personnes * coutHoraire) : null
  const complet = coutVehicule != null && coutTemps != null
  return {
    minutes: duree,
    coutVehicule,
    coutTemps,
    marge: complet ? arrondi(nombre(prix) ?? 0) - coutVehicule - coutTemps : null,
    equilibre: complet ? arrondi(coutVehicule + coutTemps) : null,
  }
}

/** Marge d'UN kilomètre facturé, temps de conduite compris. */
export function margeParKm({ tarif, coutKm = null, personnes = 1, coutHoraire = null, vitesse = null }) {
  if (coutKm == null || coutHoraire == null || !vitesse) return null
  return arrondi((nombre(tarif) ?? 0) - coutKm - personnes * coutHoraire / vitesse, 2)
}
