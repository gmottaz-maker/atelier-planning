...
/**
 * Valide un jeu de coefficients venu des réglages partagés. Renvoie toujours
 * un jeu utilisable : une valeur absente ou aberrante retombe sur la valeur
 * par défaut plutôt que de casser un chiffrage.
 */
export function normaliserComplexites(brut) {
  const out = {}
  for (const [cle, defaut] of Object.entries(COMPLEXITES_DEFAUT)) {
    const v = brut?.[cle] || {}
    const nombre = (x, repli) => {
      const n = parseFloat(x)
      return Number.isFinite(n) && n > 0 && n <= 20 ? n : repli
    }
    out[cle] = {
      label: typeof v.label === 'string' && v.label.trim() ? v.label.trim() : defaut.label,
      matiere: nombre(v.matiere, defaut.matiere),
      temps: nombre(v.temps, defaut.temps),
    }
  }
  return out
}
