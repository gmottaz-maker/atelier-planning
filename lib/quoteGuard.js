 ...
 *
 * La logistique est faite d'ÉVÉNEMENTS qui contiennent des lignes : compter
 * les entrées du tableau annoncerait « 1 ligne préservée » pour un événement
 * qui en porte douze, dans le message même qui explique ce qu'on refuse de
 * détruire. On traverse donc.
 */
export function lignesDevis(quote) {
  return SECTIONS.reduce((n, k) => {
    const liste = quote?.[k]
    if (!Array.isArray(liste)) return n
    return n + liste.reduce((m, x) => m + (Array.isArray(x?.lignes) ? x.lignes.length : 1), 0)
  }, 0)
}
