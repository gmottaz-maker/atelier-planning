...
import { MODELE_PRECIS } from './modelesClaude'
...
export async function scanReceipts({ apiKey, image, mimeType }) {
  ...
  if (!response.ok) {
    // Le statut et le corps voyagent avec l'erreur : sans eux, impossible de
    // distinguer une surcharge passagère d'un crédit épuisé (cf. lib/scanErreur.js).
    throw new ErreurClaude(response.status, await response.text())
  }
