...
async function render(html, annexe, options = {}) {
  ...
  try {
    ...
    // Le délai par défaut de Puppeteer est de 30 s, et il compte le RENDU, pas
    // seulement le réseau. Une présentation client — dix-sept pages de
    // 1920 × 1080 avec ses visuels inclus dans le document — le dépassait, et
    // l'erreur « Navigation timeout of 30000 ms exceeded » arrivait à l'écran
    // sans dire lequel des deux était lent.
    //
    // `attendre` : un document dont tout est inliné (police et images en
    // data-URI) n'a AUCUNE requête réseau à attendre ; `load` suffit et évite
    // les 500 ms d'inactivité que `networkidle0` impose en pure perte. Les
    // offres et factures, elles, vont chercher leur police chez Google et
    // gardent donc le réglage par défaut.
    const { attendre = 'networkidle0', delai = 30000 } = options
    await page.setContent(html, { waitUntil: attendre, timeout: delai })
    const principal = await page.pdf({ format: 'A4', printBackground: true, preferCSSPageSize: true, timeout: delai })
 ...
 */
export async function htmlToPdf(html, annexe, options) {
  // On s'enchaîne à la file, sans propager l'échec du rendu précédent.
  const mine = queue.then(() => render(html, annexe, options))
  queue = mine.catch(() => {})
  try {
    return await mine
  } catch (e) {
    ...
    if (String(e?.message || '').includes('ETXTBSY')) {
      ...
      const retry = queue.then(() => render(html, annexe, options))
      queue = retry.catch(() => {})
      return await retry
    }
    throw e
