...
export async function loadCandidates(supabase) {
  const [sup, cus, exp, mSup, mCus, mExp] = await Promise.all([
    ...
    // « created » (émise, pas encore envoyée) et « sent » (envoyée, en attente
    // de paiement) sont les deux états d'une facture client rapprochable.
    //
    // Cette ligne cherchait `status = 'pending'`. C'est un statut de facture
    // FOURNISSEUR : aucune facture client ne l'a jamais porté, la liste des
    // candidates était donc systématiquement vide et un virement de client ne
    // pouvait se rapprocher de rien — ni automatiquement, ni à la main dans la
    // recherche libre, qui lit la même liste.
    supabase.from('customer_invoices').select('*').in('status', ['created', 'sent']),
