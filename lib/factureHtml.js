...
// par le marqueur `hidden`. Le couple détaillé/résumé a disparu — il faisait
// double emploi et obligeait à choisir avant de savoir ce qu'on voulait montrer.
...
export function buildFactureHtml(inv, company, qrSvg) {
  // `qrSvg` ne sert qu'à décider si l'IBAN doit être rappelé en clair :
  ...
  ${objectBlock(inv.projects?.name || inv.object || '—', [
    ...
    // Celle de la facture d'abord : un chantier facturé en acompte puis en
    // solde peut porter deux bons de commande. À défaut, celle du projet.
    (inv.reference || inv.projects?.reference)
      ? `Référence : ${inv.reference || inv.projects.reference}` : null,
  ])}
  ${lineTable(rows)}
