...
// mode: 'detailed' (lignes complètes) | 'summary' (sous-totaux de section).
import { buildOfferRows } from './devisHtml'
import {
...
export function buildFactureHtml(inv, company, mode, qrSvg) {
  ...
  const rows = buildOfferRows(inv.quote_snapshot, isStorage ? { itemsLabel: 'Stockage' } : {})
