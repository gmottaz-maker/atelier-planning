...
import { genRowUid, genItemUid } from './quoteLines'
...
export function defaultQuote(reglages) {
  ...
  return {
    ...
    items: [],
    subcontracting: [],
    // Un ÉVÉNEMENT, et un seul : trajet + montage + démontage, c'est déjà un
    // passage sur le chantier. Il part SANS NOM — un nom poserait un en-tête
    // de groupe sur le PDF de toutes les offres, y compris celles qui n'ont
    // qu'un aller. On le nomme le jour où il y en a un deuxième.
    logistics: [
      { _uid: genItemUid(), nom: '', lignes: [
...
/**
 * Faut-il semer un devis vierge aux tarifs réglés ?
 *
 * Cette fonction existe parce que la condition, écrite à la main dans un
 * `useEffect`, a effacé deux offres.
 *
 * Le piège : l'effet dépendait de « l'offre a été touchée ». Enregistrer
 * remet ce drapeau à faux — donc l'effet se relançait APRÈS la sauvegarde,
 * retrouvait un projet toujours « sans offre enregistrée », et remplaçait le
 * travail par un devis vide. L'écran se vidait, puis l'enregistrement
 * automatique écrivait ce vide en base.
 *
 * D'où `dejaSeme` : on ne sème qu'UNE fois. Les autres gardes restent, mais
 * c'est celle-là qui rend le retour en arrière impossible.
 */
export function doitSemerOffreVierge({ tarifsCharges, dejaSeme, offreEnregistree, offreTouchee }) {
  if (dejaSeme) return false          // une seule fois, jamais deux
  if (!tarifsCharges) return false    // les tarifs arrivent après le premier rendu
  if (offreEnregistree) return false  // ne jamais écraser un devis existant
  if (offreTouchee) return false      // ni ce qui vient d'être tapé
  return true
}
