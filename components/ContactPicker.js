...
export default function ContactPicker({ onSelect, placeholder = 'Rechercher un contact…' }) {
  ...
  const dd = { position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: AL.white, border: `1px solid ${C.border}`, borderRadius: R.panel, border: `1.5px solid ${C.outline}`, maxHeight: 280, overflowY: 'auto', zIndex: 50 }
  ...
  return (
    <div ref={boxRef} style={{ position: 'relative' }}>
      <input value={q}
        ...
        className="w-full px-3 py-2 border u-line u-pill text-sm focus:outline-none focus:u-line u-surface" />
      {open && needle && (
        <div style={dd}>
          {matches.length === 0 ? (
            <div style={{ padding: '10px 12px', fontSize: 13, color: C.muted }}>Aucun contact.</div>
          ) : matches.map(c => {
            const co = c.kind === 'person' && c.parent_id ? byId[String(c.parent_id)] : null
            ...
            return (
              <button key={c.id} type="button" onClick={() => choose(c)}
                ...
                <span style={{ width: 26, height: 26, borderRadius: c.kind === 'company' ? 6 : '50%', background: AL.black, color: C.accentSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, flex: 'none' }}>
                  {(c.name || '?').slice(0, 2).toUpperCase()}
                </span>
                <span style={{ minWidth: 0, flex: 1 }}>
                  <span style={{ display: 'block', fontSize: 13, fontWeight: 600, color: AL.black, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</span>
                  <span style={{ display: 'block', fontSize: 11, color: C.muted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{[sub, c.email].filter(Boolean).join(' · ')}</span>
                </span>
              </button>
            )
          })}
