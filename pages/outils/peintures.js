// Peintures RUCO : choix du produit ET chiffrage du travail, en un seul outil.
//
// Deux jeux de données se rejoignent ici par le SKU :
//   • le CATALOGUE technique (/public/ruco/products.json, 271 produits) dit
//     quel produit convient à quel support, brillance et mode d'application ;
//   • le TARIF d'atelier (lib/paintPrices.js, 20 produits réellement achetés)
//     porte les prix facturés, les ratios 2K, dilutions et rendements.
//
// Un produit du catalogue dont l'un des articles a un prix devient chiffrable :
// le panneau de droite calcule alors quantités, coût matière et temps.
//
// Aucun lien avec le catalogue d'articles de l'app ni avec les devis. Les données
              ...
              <article key={p.id} style={{ ...card, borderColor: top ? C.ink : C.border, overflow: 'hidden' }}>
                <div role="button" tabIndex={0} onClick={() => setOpen(isOpen ? null : p.id)}
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ font: `600 14px ${FONT}`, color: C.ink, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      {p.name}
